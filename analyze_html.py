...
output_path = r"d:\Paginas_web\pagina\laboratorio-bienestar\analysis_result_3.txt"
...
with open(output_path, 'w', encoding='utf-8') as out:
    ...
    keywords = ["catalog-grid-item__price"]
    ...
    for keyword in keywords:
        ...
        for i, index in enumerate(matches):
            ...
            start = max(0, index - 1000) # Look further back to find parent
            end = min(len(content), index + 200)
            context = content[start:end].replace('\n', ' ').replace('\r', '')
            out.write(f"Match {i+1} at {index}:\n...{context}...\n\n")

print("Analysis complete. Check analysis_result_3.txt")
