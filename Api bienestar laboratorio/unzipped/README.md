...
**Fecha de generación**: 2026-01-22T11:51:46.837999
...
La API estará disponible en: `http://localhost:10020`
...
**Keywords Permitidas**: análisis, muestra, resultado, laboratorio, paciente, examen

**Keywords Prohibidas**: hipoteca, medicamento, proceso, pm2
...
```javascript
const response = await fetch('http://localhost:10020/api/chat', {
  method: 'POST',
...
response = requests.post('http://localhost:10020/api/chat', 
    json={
        'message': '¿Cómo puedo ayudarte?',
        'model': 'llama2'
    })
...
```bash
curl -X POST http://localhost:10020/api/chat \
...
- `PORT`: Puerto en el que correrá la API (default: 10020)
