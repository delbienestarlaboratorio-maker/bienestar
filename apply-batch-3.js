...
const batchDataFile = path.join(__dirname, 'batch3.json');
...
try {
    ...
    for (const [slug, meanings] of Object.entries(batchData)) {
        ...
        if (fs.existsSync(file)) {
            ...
            // Defensiva contra el autocompletado del modelo de AI 
            if (bm.highMeaning.includes(' contains ') || bm.highMeaning.includes(' Defines ') || bm.lowMeaning.includes(' provides ') || bm.lowMeaning.includes(' is ') || bm.lowMeaning.includes(' the ') || bm.highMeaning.includes(' The ')) {
    ...
    console.log(`✅ LOTE 3 COMPLETO: Se actualizaron con éxito ${updated} biomarcadores locales.`);
} catch (e) {
